...
por_csm = []
for csm in csm_list:
    r  = reimpl[reimpl['CSM'] == csm]
    nr = nao_reimpl[nao_reimpl['CSM'] == csm]
...
for _, row in rp.iterrows():
    rows_aulas.append({
        ...
        'empresa':     clean(row['empresa_final']),
        'csm':         clean(row['CSM']),
        'conteudo':    clean(row['Conteúdo']),
        'turma':       clean(row['Turmas']),
        'progresso':   clean(row['Progresso']),
        'matricula':   clean(row['Matrícula']),
        'data_compra': clean(row['Data da compra']),
        'mes':         row['mes'],
...
top_conteudos = sorted(
    [{'conteudo': str(c), 'total': len(df_c)} for c, df_c in rp.groupby('Conteúdo')],
    key=lambda x: -x['total']
)
meses_aulas   = sorted([m for m in rp['mes'].dropna().unique() if m])
summary_aulas = {
    'total_aulas':      len(rp),
    'unique_users':     rp['email_lower'].nunique(),
