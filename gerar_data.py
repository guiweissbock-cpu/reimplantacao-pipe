...
Requer na mesma pasta (APENAS estes 4 CSVs + o index.html):
    members-report.csv  ← export da plataforma com membros (turmas, último acesso)
...
# Turmas genéricas que NÃO representam uma empresa
GENERIC = {'Aplicativo', 'Full Pass', 'Pré-vendas', 'Gestão', 'Executivos',
           'Canais', 'Class', 'Novo grupo - Priscila', 'Forma Turismo', 'Pré Vendas'}

# Casos ambíguos de CSM (nome duplicado). Adicione aqui se surgir conflito.
CSM_OVERRIDES = {
    'totvs': 'Bárbara Cabrini',  # TOTVS BRASIL CENTRAL (Bárbara) vs TOTVS (Gustavo)
}

# ── LEITURA ROBUSTA DE CSV (detecta ; ou , automaticamente) ──
def detect_sep(path):
...
usuarios = read_csv(BASE / 'usuarios.csv')
clientes = read_csv(BASE / 'clientes.csv')
report   = read_csv(BASE / 'consumo.csv')

members['email_lower']  = members['Email'].astype(str).str.lower().str.strip()
usuarios['email_lower'] = usuarios['Url do E-mail do Membro'].astype(str).str.lower().str.strip()
report['email_lower']   = report['Email'].astype(str).str.lower().str.strip()
...
for _, r in clientes[['Empresa', 'CSM']].dropna(subset=['Empresa']).iterrows():
    company_csm[str(r['Empresa']).strip().lower()] = r['CSM']

def get_csm(name):
    if pd.isna(name) or str(name).strip() == '': return ''
    nl = str(name).strip().lower()
    if nl in CSM_OVERRIDES: return CSM_OVERRIDES[nl]
    if nl in company_csm: return str(company_csm[nl])
    for k, v in company_csm.items():
        if nl in k or k in nl: return str(v)
    return ''

# ── EXTRAÇÃO DE EMPRESA A PARTIR DO CAMPO "Turmas" ───────────
# Formatos aceitos:  "1 - 038 - Becomex"  -> Becomex   |   "52 - Totvs" -> Totvs
def extract_companies(turma):
    ...
    for p in [x.strip() for x in str(turma).split(',')]:
        m1 = re.match(r'^\d+ - \d+ - (.+)$', p)
        if m1:
            companies.append(m1.group(1).strip()); continue
        m2 = re.match(r'^\d+ - (.+)$', p)
        if m2:
...
def empresa_from_domain(email):
    ...
    domain_map = {
        ...
        'bmchyundai': 'BMC Hyundai', 'cartrom': 'CARTROM', 'medika': 'Medika',
        'tecnogera': 'Tecnogera', 'becomex': 'Becomex', 'progic': 'Progic',
...
members['acessou']        = members['Último acesso'].astype(str).str.strip() != 'Nunca acessou'
...
total_r   = len(reimpl)
acessaram = int(reimpl['acessou'].sum())
...
def to_rows(df):
    ...
    for _, r in df.iterrows():
        ...
        rows.append({'nome': clean(r.get('Nome', '')), 'email': clean(r.get('Email', '')),
                     'empresa': emp, 'csm': clean(r.get('CSM', '')),
...
for emp in empresas_reimpl:
    ...
    por_empresa.append({'empresa': emp, 'csm': csm_mode.iloc[0] if len(csm_mode) else '',
                        'total': tot, 'acessaram': acc, 'nao_acessaram': tot - acc,
...
for csm in csm_list:
    ...
    por_csm.append({'csm': csm, 'empresas': len(emps), 'total': tot, 'acessaram': acc,
                    'nao_acessaram': tot - acc, 'pct': round(acc / tot * 100, 1) if tot else 0,
                    'pendentes': len(nr)})
...
for _, row in rp.iterrows():
    rows_aulas.append({'nome': clean(row['Nome']), 'email': clean(row['Email']),
                       ...
                       'situacao': clean(row.get('Situação do membro', ''))})
...
for emp, df_e in rp[rp['empresa_final'] != ''].groupby('empresa_final'):
    ...
    por_empresa_aulas.append({'empresa': str(emp), 'csm': csm_mode.iloc[0] if len(csm_mode) else '',
                              'total_aulas': len(df_e), 'usuarios': df_e['email_lower'].nunique(),
                              'conteudos': df_e['Conteúdo'].nunique()})
por_empresa_aulas.sort(key=lambda x: -x['total_aulas'])

top_conteudos = sorted([{'conteudo': str(c), 'total': len(df_c)} for c, df_c in rp.groupby('Conteúdo')],
...
summary_aulas = {'total_aulas': len(rp), 'unique_users': rp['email_lower'].nunique(),
                 'unique_empresas': rp[rp['empresa_final'] != '']['empresa_final'].nunique(),
                 'unique_conteudos': rp['Conteúdo'].nunique()}

# ── INJETA NO HTML ───────────────────────────────────────────
now = pd.Timestamp.now().strftime('%d/%m/%Y')
js = (f'var UPD={json.dumps(now)};\n'
      + 'var SUMMARY=' + json.dumps({'empresas_reimplantadas': len(empresas_reimpl),
          'total_usuarios_reimpl': total_r, 'acessaram': acessaram,
          'nao_acessaram': total_r - acessaram,
          'pct_adocao': round(acessaram / total_r * 100, 1) if total_r else 0,
...
print(f'   Empresas reimplantadas : {len(empresas_reimpl)} | Usuarios: {total_r} | Acessaram: {acessaram}')
print(f'   Nao reimplantados      : {len(nao_reimpl)} | Nao encontrados: {len(nao_enc)}')
print(f'   Aulas registradas      : {len(rp)} | Usuarios unicos: {rp["email_lower"].nunique()} | Conteudos: {rp["Conteúdo"].nunique()}')
