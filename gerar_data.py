...
# ── LEITURA DE CSV COM DETECÇÃO AUTOMÁTICA DE SEPARADOR ─────────────────────
def read_csv_auto(path):
    """Lê CSV detectando separador (vírgula ou ponto-e-vírgula) e BOM automaticamente."""
    try:
        with open(path, 'r', encoding='utf-8-sig', errors='replace') as f:
            sample = f.read(8192)
        sep = ';' if sample.count(';') > sample.count(',') else ','
        df = pd.read_csv(path, sep=sep, encoding='utf-8-sig')
        print(f"  📄 {Path(path).name}: {len(df)} linhas, sep='{sep}', colunas: {list(df.columns)[:6]}")
        return df
